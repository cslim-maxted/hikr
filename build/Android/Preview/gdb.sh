...
PATH="/Users/chulseunglim/Library/Android/sdk/platform-tools:$PATH"
...
NDK_DIR="/Users/chulseunglim/Library/Android/sdk/ndk-bundle"

AWK_SCRIPTS=$NDK_DIR/build/awk
PID=`adb shell ps | awk -f $NDK_DIR/build/awk/extract-pid.awk -v PACKAGE="com.apps.hikr"`
DATA_DIR=/data/data/com.apps.hikr #`adb shell run-as com.apps.hikr /system/bin/sh -c pwd`
DEVICE_GDBSERVER=$DATA_DIR/lib/gdbserver

echo ""
echo "## Starting gdbserver at $DEVICE_GDBSERVER"
adb shell run-as com.apps.hikr $DEVICE_GDBSERVER +debug-socket --attach $PID &
